import { messageBadges, messageCommand, messageParameters, messageEmotes, messagePrefix, messageUser, messageContent, cleanChannelName, messageEmotesReturn, correctMessage } from "oberknecht-utils";
import { i } from "..";
...
export class privmsgMessage {
    ...
    async ban(reason?: string) { return i.OberknechtAPI[this.sym].ban(this.channelID, this.senderUserID, reason) };
    async unban() { return i.OberknechtAPI[this.sym].unban(this.channelID, this.senderUserID) };
    async timeout(duration: number, reason?: string) { return i.OberknechtAPI[this.sym].timeout(this.channelID, this.senderUserID, duration, reason) };
    ...
    async announce(message: string, color?: string) { return i.OberknechtAPI[this.sym].announce(this.channelID, message, color) };
    async slow(wait_time: number) { return i.OberknechtAPI[this.sym].slow(this.channelID, wait_time) };
    async slowOff() { return i.OberknechtAPI[this.sym].slowOff(this.channelID) };
    async followers(duration: number) { return i.OberknechtAPI[this.sym].followers(this.channelID, duration) };
    ...
    async mod(userid: string) { return i.OberknechtAPI[this.sym].mod(userid) };
    async unmod(userid: string) { return i.OberknechtAPI[this.sym].unmod(userid) };
    async vip(userid: string) { return i.OberknechtAPI[this.sym].vip(userid) };
    async unvip(userid: string) { return i.OberknechtAPI[this.sym].unvip(userid) };

    async raid(to_broadcaster_id: string) { return i.OberknechtAPI[this.sym].raid(this.channelID, to_broadcaster_id) };
    async unraid(broadcaster_id: string) { return i.OberknechtAPI[this.sym].unraid(broadcaster_id) };

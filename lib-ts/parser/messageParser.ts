...
export function messageParser(sym: string, rawMessage: string, wsnum?: number) {
    ...
    i.OberknechtEmitter[sym].emit(["irc:_message", `irc:${wsnum_}:_message`], rawMessage);
    ...
    switch (IRCCommand) {
        ...
        case "USERSTATE":
        case "WHISPER": {
            i.OberknechtEmitter[sym].emit([`irc:${IRCCommand.toLowerCase()}`, `irc:${wsnum_}:${IRCCommand.toLowerCase()}`, IRCCommand], new messageTypes[`${IRCCommand.toLowerCase()}Message`](sym, rawMessage));
            break;
        };

        case "PRIVMSG": {
            const privmsg = new messageTypes.privmsgMessage(sym, rawMessage);
            i.OberknechtEmitter[sym].emit([`irc:${IRCCommand.toLowerCase()}`, `irc:${wsnum_}:${IRCCommand.toLowerCase()}`, IRCCommand], privmsg, privmsg.channelName, privmsg.senderUserName, privmsg.messageText);
            break;
        };
        ...
        case "375": {
            (async () => {
                ...
                i.OberknechtEmitter[sym].emit(["client:ready", `irc:${wsnum_}:ready`, "irc:open", "irc:ready"], rawMessage);
            })();

export function _splitmsg(
  msg: string,
  num?: number,
  splitter?: string,
  maxsplitdistance?: number,
  insertbetweensplits?: string
) {
  if (!(msg ?? undefined)) return;
  if (!(num ?? undefined)) num = 500;
  if (!(splitter ?? undefined)) splitter = " ";
  if (!(maxsplitdistance ?? undefined)) maxsplitdistance = 50;
  if (!(insertbetweensplits ?? undefined)) insertbetweensplits = "";
  ...
  for (let i = 0; i < Infinity; i++) {
    if (m.length <= 0) break;
    let p =
      (i > 0 && m.length > 0 ? insertbetweensplits : "") +
      m.splice(0, num)?.join("");

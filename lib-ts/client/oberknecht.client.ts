...
import {
  ...
  oberknechtQueueEmitter,
} from "oberknecht-emitters";
import {
  clearchatMessage,
...
const onGLOBALUSERSTATEcallback = (
...
export class oberknechtClient {
  ...
  get options() {
    return i.clientData[this.symbol]?._options ?? {};
  }
  get clientData() {
    return i.clientData[this.symbol];
  }
  get _allIndex() {
    return i;
  }
  ...
  OberknechtActionEmitter: oberknechtActionEmitter = new oberknechtActionEmitter(
    { useExpectedEventNames: true }
  );
  ...
  get channels() {
    return i.clientData[this.symbol].channels ?? [];
  }
  get wsUptime() {
    return i.reconnectingKnechtClient[this.symbol]?.[0]?.startTime ?? undefined
      ? Date.now() - i.reconnectingKnechtClient[this.symbol]?.[0]?.startTime
      : null;
  }
  get wsNum() {
    return i.clientData[this.symbol].wsNum;
  }
  get wsConnections() {
    return i.clientData[this.symbol].wsConnections;
  }
  get isConnected() {
  ...
  constructor(options: clientOptions) {
    ...
    _options.secure = _options.secure ?? true;
    ...
    process.on("unhandledRejection", (e) => {
      this.OberknechtEmitter.emitError(
        "unhandledRejection",
        Error("unhandledRejection", { cause: e })
      );
    });
  }
  ...
  async destroy() {
    return new Promise<void>(async (resolve, reject) => {
      await Promise.all(
        Object.keys(i.reconnectingKnechtClient[this.symbol]).map(async (ws) => {
  ...
  privmsg = (channel: string, message: string) => {
    return privmsg(this.symbol, channel, message);
  };
  send = this.privmsg;
  say = this.privmsg;
  action = (channel: string, message: string) => {
    return action(this.symbol, channel, message);
  };
  sendRaw = (message: string) => {
    return sendraw(this.symbol, message);
  };
  join = (channel: string) => {
    return join(this.symbol, channel);
  };
  joinAll = (channels: string | string[]) => {
    return joinAll(this.symbol, channels);
  };
  part = (channel: string) => {
    return part(this.symbol, channel);
  };
  partAll = (channels: string | string[]) => {
    return partAll(this.symbol, channels);
  };
  ping = () => {
    return ping(this.symbol);
  };
  whisper = (targetUser: string, message: string, customtoken?: string) => {
    return whisper(this.symbol, targetUser, message, customtoken);
  };
  ...
  getuser = async (
    ...
    return await getuser(this.symbol, login, id, noautofilterids);
  };
  getusers = async (
    ...
    return getusers(this.symbol, logins, ids, noautofilterids);
  };
  ...
  onPRIVMSG = (callback: typeof onPRIVMSGcallback) => {
    this.on("PRIVMSG", callback);
  };
  onGLOBALUSERSTATE = (callback: typeof onGLOBALUSERSTATEcallback) => {
    this.on("GLOBALUSERSTATE", callback);
  };
  onCLEARCHAT = (callback: typeof onCLEARCHATcallback) => {
    this.on("CLEARCHAT", callback);
  };
  onNOTICE = (callback: typeof onNOTICEcallback) => {
    this.on("NOTICE", callback);
  };
  onUSERNOTICE = (callback: typeof onUSERNOTICEcallback) => {
    this.on("USERNOTICE", callback);
  };
  onUSERSTATE = (callback: typeof onUSERSTATEcallback) => {
    this.on("USERSTATE", callback);
  };
  onWHISPER = (callback: typeof onWHISPERcallback) => {
    this.on("WHISPER", callback);
  };
  onError = (callback: typeof onErrorcallback) => {
    this.on("error", callback);
  };
  onReady = (callback: typeof onEmptyCallback) => {
    this.on("ready", callback);
  };
  onClose = (callback: typeof onEmptyCallback) => {
    this.on("irc:close", callback);
  };
  onAutojoin = (callback: typeof onAutojoinCallback) => {

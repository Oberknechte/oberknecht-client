import { i } from "..";
import { noticeMessage } from "../parser/NOTICE.Message";

export function handleNotice(sym: string, rawMessage: string, wsnum: number) {
    let notice = new noticeMessage(sym, rawMessage);
    if (!i.clientData[sym]) return;
    wsnum = (wsnum ?? i.clientData[sym]?.currentKnecht);
    i.OberknechtEmitter[sym]?.emit(["irc:notice", `irc:${wsnum}:notice`], rawMessage);
    switch (notice.msgID) {
        case "cmds_available":
        ...
        case "whisper_restricted_recipient":
            ...
            {
                i.OberknechtEmitter[sym].emit(["irc:notice:reject"], rawMessage);
                i.OberknechtActionEmitter[sym].emitreject("PRIVMSG", rawMessage);
                break;
            }
        ...
        case "delete_staff_message_success":
            ...
            {
                i.OberknechtEmitter[sym].emit(["irc:notice:resolve", `irc:${wsnum}:notice:resolve`], rawMessage);
                i.OberknechtActionEmitter[sym].emitresolve("PRIVMSG", rawMessage);
                break;
            }
        ...
        case "bad_mod_mod":
            ...
            {
                i.OberknechtEmitter[sym].emit(["irc:notice:rejectsuccess", "irc:notice:success", `irc:${wsnum}:notice:rejectsuccess`, `irc:${wsnum}:notice:success`], rawMessage);
                break;
            }
        ...
        case "usage_whisper":
            ...
            {
                i.OberknechtEmitter[sym].emit(["irc:notice:usage", `irc:${wsnum}:notice:usage`], rawMessage);
                break;
            }
        ...
        case "subs_on":
            ...
            {
                i.OberknechtEmitter[sym].emit(["irc:notice:info", `irc:${wsnum}:notice:info`], rawMessage);
                break;
            }
        ...
        case "msg_channel_suspended":
            ...
            {
                i.OberknechtActionEmitter[sym].emitreject("353", "This channel does not exist or has been suspended.");
                break;
            }
    }
};
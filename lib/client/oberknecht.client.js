...
const knechtQueueEmitter = require("../emitters/oberknecht.queue.emitter");
...
class oberknechtClient {
    ...
    OberknechtQueueEmitter = new knechtQueueEmitter();
    
    get symbol() { return this.#symbol };
    get uptime() { return (Date.now() - this.#startTime) };
    get wsUptime() { return ((this.#wsStartTime ?? undefined) ? (Date.now() - this.#wsStartTime) : null) };
    get #ws_url() { return `ws${this.#secure ? "s" : ""}://irc-ws.chat.twitch.tv:${this.#secure ? 433 : 80}`; };
    get channels() { return i.clientData[this.symbol].channels ?? []; };

    /** @param {clientOptions} options */
    constructor(options) {
        this.#symbol = Symbol(this._options.username ?? "oberknecht");
        ...
        const API = this.API = new oberknechtAPI({ token: this._options.token });

        i.OberknechtAPI[this.symbol] = API;
    ...
    async connect() {
        ...
        reconnectingKnechtClient.addEventListener("open", (response) => {
            this.#wsStartTime = i.clientData[this.symbol].wsUptime = Date.now();

            reconnectingKnechtClient.send(`CAP REQ :twitch.tv/membership twitch.tv/tags twitch.tv/commands`);
            reconnectingKnechtClient.send(`PASS ${!this._options.anonymus ? `oauth:${this._options.token ?? ""}` : "schmoopiie"}`);
            reconnectingKnechtClient.send(`NICK ${!this._options.anonymus ? this._options.username ?? "oberknecht" : "justinfan69"}`);
        ...
        reconnectingKnechtClient.addEventListener("close", (response) => {
            this.#wsStartTime = i.clientData[this.symbol].wsUptime = null;
        ...
        reconnectingKnechtClient.addEventListener("error", (response) => {
            this.OberknechtEmitter.emitError(["ws:error", "client:error"], Error(response));
        });
    };

...
let onPRIVMSGcallback = /**@param {privmsgMessage} privmsg */ (privmsg) => { };
...
const _getclientid = require("../operations/api/_getclientid");
...
class oberknechtClient {
    ...
    constructor(options) {
        ...
        if(!this._options.clientid){
            _getclientid(this.symbol, this._options.token)
            .catch()
        };
    ...
    connect() {
        ...
        reconnectingKnechtClient.addEventListener("error", (response) => {
            let response_ = response.data;
            this.OberknechtEmitter.emit(["ws:error", "client:error", "error"], response_);
        });
    };
    ...
    whisper = (targetUserID, message) => {
        return require("../operations/whisper")(this.symbol, targetUserID, message);
    };

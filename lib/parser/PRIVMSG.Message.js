...
class privmsgMessage {
    ...
    constructor(sym, rawMessage) {
        ...
        this.IRCMessageParts = [...this._raw.split(" ").slice(0, 4), this._raw.split(" ").slice(4).join(" ").replace(/(^:|^\s|\s$)/g, "").replace(/[\s]{2,}/g, " ")];
        ...
        this.reply = (replyMessage) => {
            return require("../operations/reply")(sym, replyMessage, this.channelName, this.messageID);
        };
    
        this.send = (sendMessage) => {
            return require("../operations/privmsg")(sym, this.channelName, sendMessage);
        };
